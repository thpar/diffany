...
public abstract class EdgeOntology
{
	...
	public EdgeDefinition getOverlapEdge(EdgeDefinition refEdge, EdgeDefinition conEdge, double cutoff) throws IllegalArgumentException
	{
		...
		// the overlapping edge is only symmetrical if both original edges are
		boolean refSymm = refEdge.isSymmetrical();
		boolean conSymm = conEdge.isSymmetrical();
		boolean overlapSymm = refSymm && conSymm;
		overlap_edge.makeSymmetrical(overlapSymm);

		if (refCat.equals(conCat))
		{
			overlap_edge.setType(refCat);
			return overlap_edge;
		}
		if (isSourceChildOf(refCat,conCat))	// conCat is superclass
		{
			if (conNeg)	// there is negation -> overlap edge is negated of type subclass
			{
				overlap_edge.setType(refCat);
				return overlap_edge;
			}
			else	// there is no negation -> overlap edge is of type superclass
			{
				overlap_edge.setType(conCat);
				return overlap_edge;
			}
		}
		if (isSourceChildOf(conCat,refCat))	// refCat is superclass
		{
			if (refNeg)	// there is negation -> overlap edge is negated of type subclass
			{
				overlap_edge.setType(conCat);
				return overlap_edge;
			}
