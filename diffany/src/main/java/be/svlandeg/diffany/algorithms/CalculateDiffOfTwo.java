...
public class CalculateDiffOfTwo
{
	...
	public DifferentialNetwork calculateDiffNetwork(ReferenceNetwork reference, ConditionNetwork condition, EdgeOntology eo, NodeMapper nm, String diff_name)
	{
		...
		for (Edge edge1 : reference.getEdges())
		{
			...
			if (diff_edge_category == null) // overlapping edge!
			{
				Node sourceoverlap = new Node(nm.getConsensusName(source1, source2));
				Node targetoverlap = new Node(nm.getConsensusName(target1, target2));
				String edge_type = edge1.getType();
				boolean symmetrical = edge1.isSymmetrical();
				Edge edgeoverlap = new Edge(edge_type, sourceoverlap, targetoverlap, symmetrical);
